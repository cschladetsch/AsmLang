...
; rdi = literal index
; Saves caller scope, loads continuation scope, executes body,
; checks continuation_signal, restores caller scope.
; -----------------------------------------------------------------------
execute_continuation_impl:
    push rbp
    mov rbp, rsp
    ...
    ; switch op_list and bytecode to continuation workspace.
    ; tokenize writes to the main token_ptrs directly (not through
    ; active_token_ptrs), so we must NOT switch that pointer -- parse_tokens
    ; must read the freshly-tokenized body from the main buffer.
    ...
    ; save caller scope on context stack
    call push_context
    ...
    ; check and clear continuation_signal (resume signal exits this level)
...
.restore:
    ...
    ; restore active workspace pointers (op_list and bytecode only)
    pop rax
    mov [rel active_bytecode], rax
    pop rax
    mov [rel active_op_list], rax
...
execute_continuation_tail:
    ...
    ; switch op_list and bytecode only (same reasoning as execute_continuation_impl)
    push qword [rel active_op_list]
    push qword [rel active_bytecode]
    lea rax, [rel cont_op_list]
    mov [rel active_op_list], rax
    ...
    ; restore active workspace pointers (op_list and bytecode only, no pop_context)
    pop rax
    mov [rel active_bytecode], rax
    pop rax
    mov [rel active_op_list], rax
